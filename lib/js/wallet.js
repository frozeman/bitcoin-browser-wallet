 ...
 */
  ...
  //   {
  //     "matches": ["http://*/*","https://*/*"],
  //     "js": ["lib/js/jquery-2.0.3.min.js", "lib/js/addressFetcher.js"],
  //     "run_at": "document_end",
  //     "all_frames": false
  //   }
  // ],
    ...
    $setup.find('button.create').on('click', function(){
        ...
        if(!_.isEmpty($password.val())) {
            ...
            if($password.val() === $passwordCorfirm.val()) {
                ...
                if(importPrivKey) {
                    var privKey = $setup.find('.importPrivkey').val(),
...
var showWallet = function(){
    ...
    chrome.tabs.executeScript(null, {file:"/lib/js/vendor/underscore-min.js"}, function() {
        chrome.tabs.executeScript(null, {file:"/lib/js/vendor/jquery-2.0.3.min.js"}, function(){
            chrome.tabs.executeScript(null, { file: "/lib/js/addressFetcher.js"});
        });
    });
    // retrieve addresses
...
var getBalance = function(){
    ...
    return $.ajax({
        url: "https://blockchain.info/address/"+ publicKey,
        data: {
            format: 'json',
            limit: 0
        }
        ...
        bStore.balance = data.final_balance / DECIMAL_POINTS;
